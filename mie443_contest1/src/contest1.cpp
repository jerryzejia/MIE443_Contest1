...
int main(int argc, char **argv)
{
    ...
    while (ros::ok() && secondsElapsed <= 480)
    {
        ...
        if (!bumperRight && !bumperCenter && !bumperLeft && laserRange > 0.7)
        {
            ...
            else if (mode == 2) //Define mode 2
            {
                ...
                if (laserRange_Left - laserRange_Right > 0.2)
                    angular = (laserRange_Left - laserRange_Right) / laserRange_Left * (0.5 * angular_max);
                else if (laserRange_Right - laserRange_Left > 0.2)
                    angular = (laserRange_Right - laserRange_Left) / laserRange_Right * (-0.5 * angular_max);
        ...
        else
        {
            ...
            if (laserRange_Left - laserRange_Right > 0.2)
                angular = (laserRange_Left - laserRange_Right) / laserRange_Left * (0.75 * angular_max);
            else if (laserRange_Right - laserRange_Left > 0.2)
                angular = (laserRange_Right - laserRange_Left) / laserRange_Right * (-0.75 * angular_max);
            else
                angular = 0.0;
